...
const Home: React.FC<HomeProps> = ({ items, isLoading, searchTerm, isDarkMode, toggleTheme }) => {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter(item => {
      // 1. 取得清洗後的欄位資料（移除首尾空格以防比對失敗）
      const itemTitle = (item.title || '').trim();
      const itemDesc = (item.description || '').trim();
      const itemMajorCat = (item.majorCategory || '').trim();
      const itemType = (item.type || '').trim();
      const itemHoliday = (item.holiday || '').trim();
      const itemColors = (item.color || '').split(',').map(c => c.trim()).filter(Boolean);

      // 2. 搜尋框邏輯 (全域模糊搜尋：只有這裡使用 includes)
      const term = (searchTerm || '').toLowerCase().trim();
      const matchesSearch = !term || 
        itemTitle.toLowerCase().includes(term) ||
        itemDesc.toLowerCase().includes(term) ||
        itemMajorCat.toLowerCase().includes(term) ||
        itemType.toLowerCase().includes(term) ||
        itemHoliday.toLowerCase().includes(term) ||
        itemColors.some(c => c.toLowerCase().includes(term));

      // 3. 大分類選單 (⚠️ 改為嚴格等於比對)
      const matchesCategory = selectedCategory === 'All' || itemMajorCat === selectedCategory;

      // 4. 類型選單 (⚠️ 改為嚴格等於比對)
      const matchesType = selectedType === 'All' || itemType === selectedType;

      // 5. 顏色選單 (⚠️ 改為精準檢查顏色陣列內是否存在完全相同的字串)
      const matchesColor = !selectedColor || itemColors.includes(selectedColor);

      // 6. 節日選單 (⚠️ 改為嚴格等於比對，解決選雙 12 出現聖誕節的問題)
      const matchesHoliday = !selectedHoliday || itemHoliday === selectedHoliday;

      // 只有四個條件同時符合（AND 邏輯）才會顯示
  ...
  const formatDate = (item: PortfolioItem) => {
    if (item.date) return item.date.replace('-', '.');
    return '';
  };
  ...
  const getLightboxMediaClass = (item: PortfolioItem) => {
      ...
      if (isVid || !isEDM) return "flex items-center justify-center overflow-hidden";
  ...
  return (
    <main className="max-w-7xl mx-auto min-h-screen relative transition-colors duration-300">
      <div className="px-4 sm:px-6 lg:px-8 pt-12 pb-8 flex flex-col md:flex-row justify-between items-end gap-6">
        ...
        <div className="hidden md:flex items-center justify-center w-32 h-32 bg-brand-yellow rounded-full relative mb-2 shadow-xl shadow-brand-yellow/20">
             <div className="text-center text-brand-black">
                 <span className="block text-2xl font-black">{filteredItems.length}</span>
                 <span className="text-xs font-bold uppercase">作品數</span>
             </div>
             <div className="absolute top-0 right-0 bg-brand-black dark:bg-white text-white dark:text-brand-black p-2 rounded-full transform rotate-12 transition-colors">
            ...
            <button
                ...
                className={`px-6 py-3 rounded-full text-sm font-black tracking-wide transition-all transform active:scale-95 shadow-sm ${
                  selectedCategory === 'All' 
                    ? 'bg-brand-black text-white dark:bg-white dark:text-brand-black ring-2 ring-brand-black dark:ring-white shadow-lg' 
          ...
          <div className="flex flex-wrap gap-3 items-center">
                ...
                {hasActiveFilters && (
                  <button 
                    ...
                    className="ml-auto md:ml-2 flex items-center px-4 py-2 bg-brand-black dark:bg-white text-white dark:text-brand-black rounded-full text-xs font-bold hover:bg-gray-800 dark:hover:bg-gray-200 transition-colors shadow-sm"
                  >
      ...
      {lightboxItem && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8 animate-fade-in" onClick={() => setLightboxItem(null)}>
          <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
             {isVideo(lightboxItem.imageUrl) ? (
                <video src={`${lightboxItem.imageUrl}#t=0.001`} className="absolute inset-0 w-full h-full object-cover blur-3xl scale-110 opacity-60" autoPlay muted loop playsInline preload="metadata" />
             ) : (
                 <div className="absolute inset-0 bg-cover bg-center blur-3xl scale-110 opacity-60" style={{ backgroundImage: `url(${lightboxItem.imageUrl})` }} />
             )}
             <div className="absolute inset-0 bg-white/40 dark:bg-brand-black/60 backdrop-blur-xl transition-colors" />
          </div>

          <div className="relative z-10 w-full max-w-7xl h-[85vh] md:h-[90vh] bg-white dark:bg-brand-dark-card rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col md:flex-row border border-white/50 dark:border-white/10 transition-colors" onClick={(e) => e.stopPropagation()}>
             <button onClick={() => setLightboxItem(null)} className="absolute top-4 right-4 md:top-6 md:right-6 z-[20] bg-white/50 dark:bg-brand-black/50 hover:bg-white dark:hover:bg-brand-black text-brand-black dark:text-white p-3 rounded-full backdrop-blur-md transition-all hover:rotate-90 shadow-sm border border-black/5 dark:border-white/10">
                <X size={24} />
             </button>
             ...
             <div className={`w-full md:w-[65%] h-[40vh] md:h-full bg-gray-50/50 dark:bg-black/20 relative ${getLightboxMediaClass(lightboxItem)}`}>
                {isVideo(lightboxItem.imageUrl) ? (
                    <div className="w-full h-full flex items-center justify-center p-4 md:p-10">
                        <video src={`${lightboxItem.imageUrl}#t=0.001`} className="max-w-full max-h-full rounded-2xl shadow-sm outline-none object-contain" controls autoPlay preload="metadata" />
                    </div>
                ) : (
                    <img src={lightboxItem.imageUrl} alt={lightboxItem.title} className={lightboxItem.type === 'EDM' ? "w-full h-auto min-h-full object-cover" : "max-w-full max-h-full object-contain p-4 md:p-10"} />
                )}
             </div>
                ...
                <div className="p-8 md:p-12 space-y-8">
                    ...
                    <div className="space-y-6">
                        {lightboxItem.majorCategory && (
                            <div className="space-y-2">
                                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1"><Layers size={12}/> 大分類</h3>
                                <button onClick={() => handleTagClick('majorCategory', lightboxItem.majorCategory!.trim())} className="text-sm font-bold bg-brand-black dark:bg-white text-white dark:text-brand-black px-4 py-2 rounded-full hover:bg-gray-800 dark:hover:bg-gray-200 transition-all active:scale-95">
                                    {lightboxItem.majorCategory}
                                </button>
                            </div>
                        )}
                        ...
                        {lightboxItem.type && (
                            <div className="space-y-2">
                                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1"><Tag size={12}/> 類型</h3>
                                <button onClick={() => handleTagClick('type', lightboxItem.type!.trim())} className="text-sm font-bold bg-brand-yellow text-brand-black px-4 py-2 rounded-full hover:brightness-110 transition-all active:scale-95 shadow-sm shadow-brand-yellow/20">
                                    {lightboxItem.type}
                                </button>
                            </div>
                        )}
                        ...
                        {lightboxItem.holiday && (
                            <div className="space-y-2">
                                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1"><Hash size={12}/> 節日</h3>
                                <button onClick={() => handleTagClick('holiday', lightboxItem.holiday!.trim())} className="text-sm font-bold border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-black/30 text-brand-black dark:text-white px-4 py-2 rounded-full hover:bg-white dark:hover:bg-brand-dark-card hover:border-brand-black dark:hover:border-white transition-all active:scale-95">
                                    {lightboxItem.holiday}
                                </button>
                            </div>
                        )}
                        ...
                        {lightboxItem.color && (
                             <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {lightboxItem.color.split(',').map(c => c.trim()).filter(Boolean).map(c => (
                                        <button key={c} onClick={() => handleTagClick('color', c)} className="group flex items-center gap-2 px-3 py-1.5 rounded-full border border-gray-100 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-500 transition-colors bg-white dark:bg-brand-dark-card shadow-sm">
                                            <span className="w-4 h-4 rounded-full border border-black/5" style={{ backgroundColor: COLOR_HEX_MAP[c] || '#e5e7eb' }} />
                                            <span className="text-xs font-bold text-gray-600 dark:text-gray-300 group-hover:text-brand-black dark:group-hover:text-white">{c}</span>
                                        </button>
                                    ))}
                                </div>
                            </div>
                    ...
                    {lightboxItem.description && (
                        <>
                             ...
                             <p className="text-gray-600 dark:text-gray-300 leading-relaxed font-medium">{lightboxItem.description}</p>
                        </>
                    )}
                </div>
             </div>
